        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header text-center">
                    <h3 class="card-title text-uppercase mb-0">Update Category</h3>
                </div>
                <div class="card-body">
                    {!! Form::open(['role' => 'form', 'action' => ['App\Http\Controllers\CategoryController@update', $category_id], 'method' => 'POST']) !!}
                    @csrf
                    ...
                    {!! Form::close() !!}
                        {!! Form::open(['role' => 'form', 'action' => ['App\Http\Controllers\CategoryController@destroy', $category_id], 'method' => 'POST']) !!}
                            <button type="submit" class="btn btn-block bg-danger text-white mb-2">{{ __('Delete') }}</button>
                        {!! Form::close() !!}
                        <a type="submit" class="btn btn-block bg-primary text-white mb-2" href="/categories">{{ __('Cancel') }}</a>
                </div>
            </div>
        </div>
    </div>
